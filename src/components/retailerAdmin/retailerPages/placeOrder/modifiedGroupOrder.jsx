...
export default function ModifiedGroupOrder() {
    ...
    const [statusName, setStatusname] = useState("Modified");
    const count = Math.ceil(docLength / limit);

    const fetchData = async (page, limit, code, statusName, name) => {
        const res = await axiosInstance.post(`/groupOrders/fetchPaginate/?page=${page}&limit=${limit}&retailer_code=${code}&order_status=${statusName}&name=${name}`, {
    ...
    useEffect(() => {
        fetchData(page, limit, code, statusName, name);
    }, [page]);
    ...
    const searchFirstChange = async (event) => {
        ...
        name = value;
        setName(value);
        fetchData(1, limit, code, statusName, value.trim());
    };
        ...
        <main className="main-panel">
            <div className="content-wrapper pd-1r">
                <div className="dashboard-top-icon">
                    <Link to="/retailer/newCustomer" className="">
                        {" "}
            ...
            <div className="content-wrapper mt-3 pd-1r">
                <div className="topNav">
                    ...
                    <Link to="/retailer/modifiedGroupOrder" className="active">Modified</Link>
                        ...
                        <tbody>
                            {orders !== null && orders.length > 0 ? orders.map((order, i) => (
                                <tr key={i}>
                                    ...
                                    <td> <Link to=""><Button className="Eyebtn"><BorderColorIcon /></Button></Link></td>
                                </tr>
                            )) :
                                <tr>
                                    <td>
