...
export default function ManageGroupFeaturesEdit() {
  ...
  useEffect(() => {
    const fetchGroupFeature = async () => {
      const res = await axiosInstance.post("groupFeatures/fetch/" + id, {
        token: user.data.token,
      });
      setProductId(res.data.data.product_id);
      setThaiName(res.data.data.thai_name);
      setName(res.data.data.name);
      setAdditional(res.data.data.additional);
      setFeatureId(res.data.data.feature_id)

      const res1 = await axiosInstance.post("/product/fetch/" + res.data.data.product_id, {
        token: user.data.token,
      });
      setFeatures(res1.data.data[0].features);
      setProduct(res1.data.data[0].name);
    };
    fetchGroupFeature();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if(name =="" || thaiName==""){
      ...
      const res = await axiosInstance.post("/groupFeatures/update/" + id, fea)
      if(res.data.status==true){
        setOpen(true)
        setSuccess(true)
        setTimeout(()=>{
          navigate("/admin/GroupStyle")
        },500)
      }else{
        setOpen(true)
        setError(true)
                        ...
                        <li key={i} style={{listStyleType:"none"}}>
                          <label className="full_label">
                            <input
                              ...
                              id={`chk${data.name}`}
                              value={data._id}
                              checked={featureId.includes(data._id)}
